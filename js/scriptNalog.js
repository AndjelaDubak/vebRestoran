    ...
    function inicijalizuj() {
        ...
        if(mojiRecepti != null) {
            for(let i=1; i<mojiRecepti.length; i++) {
                ...
                var slika = $("<div/>").attr("class", 'col-lg-6 col-md-6').append($("<div/>").attr("class", 'categories__post__item__pic set-bg').css('background-image', "url(" + mojiRecepti[i].pocetnaSlika ));
    
                var tekst = $("<div/>").attr("class", 'col-lg-6 col-md-6').append($("<div/>").attr("class", 'categories__post__item__text').append($("<ul/>").attr("class", 'post__label--large').append($("<li/>").append($("<br>")).append($("<br>")).append("" + mojiRecepti[i].grupa))).append($("<h3/>").append("" + mojiRecepti[i].ime).css('font-size', '30px').append($("<ul/>").attr("class", "post__widget").append($("<li/>").append("trajanje ").css({'list-style' : 'none', 'font-size' : '12px', 'color' : '#888888'}).append($("<span/>").append(mojiRecepti[i].trajanje).css('color', 'black'))).append($("<li/>").append("nivo ").css({'list-style' : 'none', 'font-size' : '12px', 'color' : '#888888'}).append($("<span/>").append(mojiRecepti[i].tezina).css('color', 'black')))).append($("<p/>").append("" + mojiRecepti[i].uputstvo)).append($("<button>").attr('class', 'site-btn').attr('id', 'brisanje' + i).append("OBRIsI RECEPT"))));
        ...
        if(kom != null) {
            $("#recepti").append($("<div/>").attr("class", 'widget__title').append($("<h4/>").append("Moji komentari:")));
        ...
        if(glasovi != null) {
            $("#recepti").append($("<div/>").attr("class", 'widget__title').append($("<h4/>").append("Moji glasovi:").attr('id', 'app')));
